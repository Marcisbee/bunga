...
export function GenericEdgeComponent({ edge }: GenericEdgeComponentProps) {
  const { input, connectableTo, output } = useStore(edge);
              ...
              <span>
                {!connectableTo[key] ? (
                  <input
                    ...
                    onChange={(event) => input[key] = parseInt(event.target.value, 10)}
                    style={{ fontSize: 11, width: 80, padding: 1, border: 0, borderRadius: 2, backgroundColor: '#fff', fontWeight: 'bold' }}
                  />
                ) : (
                  // @TODO:
                  <strong>{input[key] && getExomeId(input[key]) && null}</strong>
                )}
              </span>
            </li>
          ))}
        ...
        <div className={styles.outputGroup}>
          {Object.keys(output).map((key) => (
            <div key={`edge-o-${getExomeId(edge)}-${key}`}>
              ...
              <span><EdgeOutput edge={edge} /></span>
              {/* <small>{output[key].to.map((e) => getExomeId(e[1])).join(', ')}</small> */}
              <span
...
function EdgeOutput({ edge }: { edge: Edge }) {
